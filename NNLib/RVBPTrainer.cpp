...
RVBPTrainer::RVBPTrainer(){
}

void RVBPTrainer::setTargetNeurons(vector<vector<int>> target){
    targetNeurons = target;
}

void RVBPTrainer::setTrainingData(rvmatrix data){
    trainingData = data;
}

void RVBPTrainer::train(Network *n){
    ...
    for (int cy_ind = 0; cy_ind<cycles; cy_ind++){
        ...
        for (int dat_ind = 0; dat_ind<trainingData.size(); dat_ind++){
            ...
            for(int la_ind = (int)network->layers.size()-1; la_ind>=0; la_ind--){
                ...
                //if(la_ind == (network->layers.size()-1)) network->layers.at(la_ind)->lerror = 0;
                
                /*
                for (int t=0; t<network->layers.at(la_ind)->neurons.size(); t++) {
                    network->layers.at(la_ind)->neurons.at(t)->printWeights();
                }
                */
                ...
                for (int neu_ind=0; neu_ind<neurons_in_layer; neu_ind++){
                    ...
                    threads.push_back(thread([=]() -> void {
                        ...
                        double result = network->layers.at(la_ind)->neurons.at(neu_ind)->activate().real();
                        double count = network->layers.at(la_ind)->neurons.at(neu_ind)->count().real();
                        ...
                        for (int m = 0; m<network->layers.at(la_ind)->neurons.at(neu_ind)->inputLayer.size(); m++){
                            ...
                            double weight_delta = learningRate * err * network->layers.at(la_ind)->neurons.at(neu_ind)->inputLayer.at(m).real();
...
double RVBPTrainer::errorDerivative(double val){
    double ret = 0;
